...
(: Small edits done by Andrew Wanczowski for generic nodes being passed into JSON functions :)
...
) as xs:string
{
    let $json :=
        ...
        then string-join(json:processElement($element/node()), "")
...
) as element(item)
{
    ...
    return
        <item type="object">{
            ...
            return
                if($pos mod 2 != 0)
                then element { xs:QName( json:escapeNCName($key) ) } { json:untypedToJSONType($keyValues[$pos + 1])/(@*, node()) }
                else ()
        }</item>
};
...
) as element(item)
{
    ...
    return
        ...
        else <item normalized-date="{ $date }" type="date">{ $value }</item>
};
...
) as xs:anyAtomicType*
{
    ...
    let $key := xs:QName( json:escapeNCName($key) )
    return
        if($type = "date")
...
) as element(item)
{
    <item>{
        ...
        then
            ...
            else if($value instance of document-node() and exists($value/json))
            then $value/json/(@*, node())
    ...
    let $items :=
        ...
        return
            ...
            then xdmp:set($finalLocation, $index)
                ...
                return <item>{ $value/(@type, @boolean, @normalized-date), $value/node() }</item>
    ...
    let $items :=
        ...
        return
            ...
            then xdmp:set($finalLocation, $index)
                ...
                return element { xs:QName( $key ) } { $value/(@type, @boolean, @normalized-date), $value/node() }
